...
async def async_create_entry(hass, entry, data):
    ...
    try:
        payload, _, _, _ = await async_prepare_entry(entry, data)
        return await async_write_entry(entry, payload, request_id)
...
async def async_prepare_entry(entry, data):
    """Read mutable eligibility and validate a detached, guarded payload."""
    ...
    if (
        ...
        raise ServiceValidationError("Tracker must be an active manual tracker in this workspace")
    if (
        ...
        raise ServiceValidationError("Subject must be active and currently assigned to the tracker")
    ...
    return payload, tracker, subject, schema


async def async_write_entry(entry, payload, request_id):
    """Send an already validated payload; retries preserve every byte and the key."""
    runtime = entry.runtime_data
    created = await runtime.api.create_entry(
        runtime.coordinator.store.workspace_id, payload, idempotency_key=request_id
    )
